...
import '../model/technician/assigned_task_model.dart';
...
class TechnicianRepositoryImpl extends TechnicianRepository {
  ...
  @override
  Future<Either<String, AssignedTaskResponseModel>> getAssignedTechnicianComplaint(AssignedTaskParams params) async {
    Either<ApiFailure, Response> result =
    await sl<TechnicianApiService>().getAssignedTechnicianComplaint(params);
    ...
    return result.fold(
        ...
        return Left(error.message);
          ...
          (data) {
        try {
          final responseModel = AssignedTaskResponseModel.fromJson(data.data);
          return Right(responseModel);
        } catch (e) {
          return Left('Failed to parse response: ${e.toString()}');
        }
